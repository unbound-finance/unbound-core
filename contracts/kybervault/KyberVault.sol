...
import '../libraries/KyberDMMPriceProvider.sol';
...
contract KyberVault is UnboundVaultBase {
    ...
    event Stake(address _user, address _wallet, uint256 _amount);
    event Unstake(address _user, address _wallet, uint256 _amount);
     ...
     * @param _kyberFactory Address of the kyber dmm factory contract
     */
    constructor(
        address _uToken,
        address _governance,
        ...
        require(
            IDMMFactory(_kyberFactory).isPool(
                IERC20_0(pair.token0()),
                IERC20_0(pair.token1()),
                _pair
            ),
            'INP'
        );
    ...
    function _lock(
        ...
        int256 price = KyberDMMPriceProvider.latestAnswer(
            address(pair),
            decimals,
            feeds,
            isBase,
    ...
    function stakeLP(
        ...
        emit Stake(msg.sender, yieldWallet[msg.sender], _amount);
    }
