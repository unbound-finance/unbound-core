...
import {IKyberFairLaunch} from '../../interfaces/IKyberFairLaunch.sol';
import {IKyberRewardLocker} from '../../interfaces/IKyberRewardLocker.sol';
...
contract KyberFairLaunch is IKyberFairLaunch, PermissionAdmin, ReentrancyGuard {
  ...
  IKyberRewardLocker private immutable _rewardLocker;
  ...
  constructor(
    ...
    IKyberRewardLocker rewardLocker_
  ) PermissionAdmin(_admin) {
    rewardTokens = _rewardTokens;
    _rewardLocker = rewardLocker_;

    // approve allowance to reward locker
    for(uint256 i = 0; i < _rewardTokens.length; i++) {
      if (_rewardTokens[i] != address(0)) {
        IERC20(_rewardTokens[i]).safeApprove(address(rewardLocker_), type(uint256).max);
      }
    }
  }
  ...
  function harvestMultiplePools(uint256[] calldata _pids) external override {
    ...
    for(uint256 i = 0; i < totalRewards.length; i++) {
      if (totalRewards[i] > 0) {
        _lockReward(IERC20(rTokens[i]), account, totalRewards[i]);
      }
    }
  }
  ...
  function _updateUserReward(
    ...
    for(uint256 i = 0; i < rTokensLength; i++) {
      ...
      if (shouldHarvest && _pending > 0) {
        _lockReward(IERC20(rewardTokens[i]), _to, _pending);
   ...
   */
  function _lockReward(IERC20 token, address _account, uint256 _amount) internal {
    uint256 value = token == IERC20(0) ? _amount : 0;
    _rewardLocker.lock{ value: value }(token, _account, _amount);
  }

  function rewardLocker() external override view returns(address){
    return address(_rewardLocker);
  }
}