...
import './interfaces/IPangolinYieldWalletFactory.sol';

contract PangolinYieldWallet {
    ...
    constructor(
        ...
        address stakingToken = IMiniChefV2(_farming).lpToken(_pid);

        require(stakingToken == _pair, 'IP');
        ...
        rewardToken = IERC20(IMiniChefV2(_farming).REWARD());
    ...
    function deposit(uint256 _amount) external onlyVault {
        IMiniChefV2(farming).deposit(pid, _amount, address(this));
    ...
    function withdraw(uint256 _amount) external onlyVault {
        IMiniChefV2(farming).withdraw(pid, _amount, address(this));

        // Send LPs to vault
        _withdrawFunds(IERC20(pair), vault, _amount);
        emit Withdraw(pid, _amount);
    }
    ...
    function getReward() external onlyOwner {
        ...
        IMiniChefV2(farming).harvest(pid, address(this));
    ...
    {
        return IMiniChefV2(farming).userInfo(pid, address(this));
    }
    ...
    function getPendingRewards() external view returns (uint256) {
        return IMiniChefV2(farming).pendingReward(pid, address(this));
    }
    ...
    function _withdrawFunds(
        ...
        if(address(_token) == pair){
            ...
            uint256 teamSharePercentage = IPangolinYieldWalletFactory(factory).teamShare();
