...
//interface
import './interfaces/IMasterChefSpookySwap.sol';
import './interfaces/ISpookySwapYieldWalletFactory.sol';

contract SpookySwapYieldWallet {
    ...
    constructor(
        ...
        IERC20 lpToken = IMasterChefSpookySwap(_farming).lpToken(_pid);
        ...
        rewardToken = IERC20(IMasterChefSpookySwap(_farming).BOO());
    ...
    function deposit(uint256 _amount) external onlyVault {
        IMasterChefSpookySwap(farming).deposit(pid, _amount);
    ...
    function withdraw(uint256 _amount) external onlyVault {
        IMasterChefSpookySwap(farming).withdraw(pid, _amount);
    ...
    function getWalletInfo()
        external
        view
        returns (IMasterChefSpookySwap.UserInfo memory)
    {
        return IMasterChefSpookySwap(farming).userInfo(pid, address(this));
    }
    ...
    function getPendingRewards() external view returns (uint256) {
        return IMasterChefSpookySwap(farming).pendingBOO(pid, address(this));
    }
    ...
    function _withdrawFunds(
        ...
        if(address(_token) == pair){
            ...
            uint256 teamSharePercentage = ISpookySwapYieldWalletFactory(factory).teamShare();
