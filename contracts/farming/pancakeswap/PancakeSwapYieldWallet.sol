...
//interface
import '../../interfaces/IMasterChefPancake.sol';
import './interfaces/IPancakeSwapYieldWalletFactory.sol';

contract PancakeSwapYieldWallet {
    ...
    function deposit(uint256 _amount) external onlyVault {
        IMasterChefPancake(farming).deposit(pid, _amount);
    ...
    function withdraw(uint256 _amount) external onlyVault {
        IMasterChefPancake(farming).withdraw(pid, _amount);
    ...
    function getWalletInfo()
        external
        view
        returns (IMasterChefPancake.UserInfo memory)
    {
        return IMasterChefPancake(farming).userInfo(pid, address(this));
    }
    ...
    function getPendingRewards() external view returns (uint256) {
        return IMasterChefPancake(farming).pendingCake(pid, address(this));
    }
    ...
    function _withdrawFunds(
        ...
        if(address(_token) == pair){
            ...
            uint256 teamSharePercentage = IPancakeSwapYieldWalletFactory(factory).teamShare();
