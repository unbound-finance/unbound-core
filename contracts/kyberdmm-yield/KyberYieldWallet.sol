...
contract KyberYieldWallet{
    ...
    constructor(
        ...
        (,address stakeToken,,,,,)= IKyberFairLaunch(_farming).getPoolInfo(_pid);
    ...
    */
        ...
        returns(uint256[] memory vestedAmounts) 
    {
        ...
        vestedAmounts = IKyberRewardLocker(rewardLocker).vestCompletedSchedulesForMultipleTokens(tokens);
     ...
     */
    function vestScheduleForMultipleTokensAtIndices(IERC20[] calldata tokens, uint256[][] calldata indices) 
        external 
        returns(uint256[] memory vestedAmounts) 
    {

        return IKyberRewardLocker(rewardLocker).vestScheduleForMultipleTokensAtIndices(tokens, indices);
     ...
     */
    function vestCompletedSchedules(IERC20 token) external returns(uint256 totalVestedAmount) {

        totalVestedAmount =  IKyberRewardLocker(rewardLocker).vestCompletedSchedules(token);
     ...
     */
    function vestScheduleAtIndices(IERC20 token, uint256[] memory indexes) external returns(uint256 totalVestedAmount) {

        totalVestedAmount = IKyberRewardLocker(rewardLocker).vestScheduleAtIndices(token, indexes);
     ...
     */
    function vestSchedulesInRange(IERC20 token, uint256 startIndex, uint256 endIndex) external returns(uint256 totalVestedAmount) {

        totalVestedAmount = IKyberRewardLocker(rewardLocker).vestSchedulesInRange(token, startIndex, endIndex);
    ...
    function numVestingSchedules(IERC20 token) external view returns(uint256) {
        ...
        return IKyberRewardLocker(rewardLocker).numVestingSchedules(address(this), token);
     ...
     * @param token Reward token (knc token) instance
     */
    function getVestingSchedules(IERC20 token) external view returns(IKyberRewardLocker.VestingSchedule[] memory) {

        return IKyberRewardLocker(rewardLocker).getVestingSchedules(address(this), token);
     ...
     */
    function getVestingScheduleAtIndex(IERC20 token, uint256 index) external view returns(IKyberRewardLocker.VestingSchedule memory) {

        return IKyberRewardLocker(rewardLocker).getVestingScheduleAtIndex(address(this), token, index);
