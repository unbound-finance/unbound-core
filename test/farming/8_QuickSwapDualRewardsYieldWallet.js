...
describe('QuickSwapDualRewardsYieldWallet', function () {
  beforeEach(async function () {
    ...
    wMaticToken = await TestToken.deploy("Wrapped MATIC", "WMATIC", 18, signers[0].address)

    let currentBlock = await ethers.provider.getBlockNumber()
    let timestamp = (await ethers.provider.getBlock(currentBlock)).timestamp

    let StakingDualRewardsFactory = await ethers.getContractFactory('StakingDualRewardsFactory')
    rewardFactory = await StakingDualRewardsFactory.deploy(timestamp + 10)

    let QuickSwapDualRewardsYieldWalletFactory = await ethers.getContractFactory(
      'QuickSwapDualRewardsYieldWalletFactory'
    )
    yieldWalletFactory = await QuickSwapDualRewardsYieldWalletFactory.deploy(rewardFactory.address)
  ...
  describe('#constructor', async () => {
    ...
    it('should set reward token contract address', async function () {
      expect(await yieldwalletInstance.rewardsTokenA()).to.be.equal(dQuickToken.address)
    })

    it('should set reward token contract address', async function () {
      expect(await yieldwalletInstance.rewardsTokenB()).to.be.equal(wMaticToken.address)
    })
  })
  ...
  describe('#deposit', async () => {
    ...
    it('lock - should emit deposit event while locking LPTs', async function () {
      ...
      let StakingDualRewards = await ethers.getContractFactory(
        'StakingDualRewards'
      )
      stakingContract = new ethers.Contract(
        stakingContract,
        StakingDualRewards.interface.fragments,
        signers[0]
      )
  ...
  describe('#getReward', async () => {
    beforeEach(async function () {
      ...
      await wMaticToken.transfer(rewardFactory.address, "200000000000000000000");
    ...
    it('unlock - should transfer dQuick reward to user', async function () {
      ...
      expect(reward).to.emit(wMaticToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "133333333333333333")
      expect(reward).to.emit(wMaticToken, "Transfer").withArgs(wallet, signers[0].address, "533333333333333333")
      ...
      expect(reward).to.emit(yieldWallet, "WithdrawFund").withArgs(wMaticToken.address, yieldWalletFactory.address, "133333333333333333");
    ...
    it('unlock - should emit reward paid event', async function () {
      ...
      expect(reward).to.emit(stakingContract, "RewardPaid").withArgs(wallet, dQuickToken.address, "333333333333333333")
      expect(reward).to.emit(stakingContract, "RewardPaid").withArgs(wallet, wMaticToken.address, "666666666666666666")
    })
  })
  ...
  describe('#getWalletInfo', async () => {
    ...
    it('should return correct yield wallet info', async function () {
      ...
      expect(info.stakedAmount.toString()).to.be.equal(lockAmount);
      expect(info.earnedA.toString()).to.be.equal("0");
      expect(info.earnedB.toString()).to.be.equal("0");
