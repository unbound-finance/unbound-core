...
describe('PancakeSwapYieldWallet', function () {
  ...
  describe('#constructor', async () => {
    ...
    beforeEach(async function () {
      ...
      let PancakeSwapYieldWallet = await ethers.getContractFactory(
        'PancakeSwapYieldWallet'
      )
      yieldwalletInstance = new ethers.Contract(
        wallet,
        PancakeSwapYieldWallet.interface.fragments,
        signers[0]
      )
    })
    ...
    it('should set reward token contract address', async function () {
      expect(await yieldwalletInstance.rewardToken()).to.be.equal(cakeToken.address)
    })
  })
  ...
  describe('#deposit', async () => {
    ...
    it('lock - should transfer reward sushi token to user if pending while staking ', async function () {
      ...
      expect(stake).to.emit(cakeToken, "Transfer").withArgs(masterchef.address, wallet, "300000000000000000000")
      expect(stake).to.emit(cakeToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "60000000000000000000")
      expect(stake).to.emit(cakeToken, "Transfer").withArgs(wallet, signers[0].address, "240000000000000000000")
      ...
      expect(stake).to.emit(yieldWallet, "WithdrawFund").withArgs(cakeToken.address, yieldWalletFactory.address, "60000000000000000000");
      expect(stake).to.emit(yieldWallet, "WithdrawFund").withArgs(cakeToken.address, signers[0].address, "240000000000000000000");
    })
  ...
  describe('#withdraw', async () => {
    ...
    it('unlock - should transfer sushi reward to user while unstaking', async function () {
      ...
      expect(unstake).to.emit(cakeToken, "Transfer").withArgs(masterchef.address, wallet, "500000000000000000000")
      expect(unstake).to.emit(cakeToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "100000000000000000000")
      expect(unstake).to.emit(cakeToken, "Transfer").withArgs(wallet, signers[0].address, "400000000000000000000")
      ...
      expect(unstake).to.emit(yieldWallet, "WithdrawFund").withArgs(cakeToken.address, yieldWalletFactory.address, "100000000000000000000");
      expect(unstake).to.emit(yieldWallet, "WithdrawFund").withArgs(cakeToken.address, signers[0].address, "400000000000000000000");
    })
  })
  ...
  describe('#claim', async () => {
    ...
    it('should transfer token to user account', async function () {
      ...
      await cakeToken.transfer(wallet, "100")
  
      let claim = await yieldwallet.claim(cakeToken.address, signers[0].address)
      
      expect(claim).to.emit(cakeToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "20");
      expect(claim).to.emit(cakeToken, "Transfer").withArgs(wallet, signers[0].address, "80");

      expect(claim).to.emit(yieldwallet, "WithdrawFund").withArgs(cakeToken.address, yieldWalletFactory.address, "20");
      expect(claim).to.emit(yieldwallet, "WithdrawFund").withArgs(cakeToken.address, signers[0].address, "80");
    })
    ...
    it('should emit claim token event', async function () {
      ...
      await expect(
        yieldwallet
          .claim(cakeToken.address, signers[0].address)
      ).to.emit(yieldwallet, "Claim")
      .withArgs(cakeToken.address, signers[0].address, "80");
    })
  })
