...
describe('PangolinYieldWallet', function () {
  beforeEach(async function () {
    ...
    pngToken = await TestToken.deploy("Pangolin Token", "PNG", 18, signers[0].address)

    let MiniChefV2 = await ethers.getContractFactory('MiniChefV2')
    miniChef = await MiniChefV2.deploy(pngToken.address, signers[0].address)

    // await miniChef.addPool("4000", ethDaiPair.address, zeroAddress);

    let PangolinYieldWalletFactory = await ethers.getContractFactory(
      'PangolinYieldWalletFactory'
    )
    yieldWalletFactory = await PangolinYieldWalletFactory.deploy(miniChef.address)
  ...
  describe('#constructor', async () => {
    ...
    beforeEach(async function () {
      ...
      let PangolinYieldWallet = await ethers.getContractFactory(
        'PangolinYieldWallet'
      )
      yieldwalletInstance = new ethers.Contract(
        wallet,
        PangolinYieldWallet.interface.fragments,
        signers[0]
      )
    })
    ...
    it('should set correct farmin contract address', async function () {
      expect(await yieldwalletInstance.farming()).to.be.equal(miniChef.address)
    })

    it('should set correct pid contract address', async function () {
      expect(await yieldwalletInstance.pid()).to.be.equal("0")
    })

    it('should set factory contract address', async function () {
      expect(await yieldwalletInstance.factory()).to.equal(yieldWalletFactory.address)
    })

    it('should approve proper allowance to farming contract', async function () {
      let stakingContract = await yieldwalletInstance.farming();
    ...
    it('should set reward token contract address', async function () {
      expect(await yieldwalletInstance.rewardToken()).to.be.equal(pngToken.address)
    })
  })
  ...
  describe('#deposit', async () => {
    ...
    it('should deposit event on stake LPT', async function () {
      ...
      await expect(stake).to.emit(yieldwallet, "Deposit").withArgs(0, lockAmount)
    ...
    it('lock - should emit deposit event while locking LPTs', async function () {
      ...
      await expect(stake).to.emit(miniChef, "Deposit").withArgs(wallet, 0, lockAmount, wallet)
      await expect(stake).to.emit(yieldwalletInstance, "Deposit").withArgs(0, lockAmount)
    ...
    it('lock - should emit proper transfer event while locking LPTs', async function () {
      ...
      await expect(lock).to.emit(ethDaiPair, "Transfer").withArgs(signers[0].address, ethDaiVault.address, lockAmount)
      await expect(stake).to.emit(ethDaiPair, "Transfer").withArgs(ethDaiVault.address, wallet, lockAmount)
      await expect(stake).to.emit(ethDaiPair, "Transfer").withArgs(wallet, stakingContract, lockAmount)
    ...
    it('lockWithPermit - should emit deposit event while staking LPTs', async function () {
      ...
      await expect(stake).to.emit(miniChef, "Deposit").withArgs(wallet, 0, permitAmount, wallet)
      await expect(stake).to.emit(yieldwalletInstance, "Deposit").withArgs(0, permitAmount)  
    })
  ...
  describe('#withdraw', async () => {
    ...
    it('unlock - should emit withdraw event while unstaking LPTs', async function () {
      ...
      await expect(unstake).to.emit(miniChef, "Withdraw").withArgs(wallet, 0, collateral, wallet)
      await expect(unstake).to.emit(yieldwalletInstance, "Withdraw").withArgs(0, collateral)
    ...
    it('unlock - should emit proper transfer event while unlock LPTs', async function () {
      ...
      await expect(unstake).to.emit(ethDaiPair, "Transfer").withArgs(stakingContract, wallet, collateral)
      await expect(unstake).to.emit(ethDaiPair, "Transfer").withArgs(wallet, ethDaiVault.address, collateral)
      await expect(unlock).to.emit(ethDaiPair, "Transfer").withArgs(ethDaiVault.address, signers[0].address, collateral)
    ...
    //   expect(unstake).to.emit(pngToken, "Transfer").withArgs(miniChef.address, wallet, "500000000000000000000")
    //   expect(unstake).to.emit(pngToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "100000000000000000000")
    //   expect(unstake).to.emit(pngToken, "Transfer").withArgs(wallet, signers[0].address, "400000000000000000000")
    ...
    //   expect(unstake).to.emit(yieldWallet, "WithdrawFund").withArgs(pngToken.address, yieldWalletFactory.address, "100000000000000000000");
    //   expect(unstake).to.emit(yieldWallet, "WithdrawFund").withArgs(pngToken.address, signers[0].address, "400000000000000000000");
    // })
  })
  ...
  describe('#getReward', async () => {
    beforeEach(async function () {
      ...
      await miniChef.addPool("4000", ethDaiPair.address, zeroAddress);
      await pngToken.transfer(miniChef.address, "300000000000000000000000");
    ...
    it('unlock - should transfer DFYN reward to user', async function () {
      ...
      let stakingContract = await yieldWallet.farming();


      await pngToken.approve(miniChef.address, "1000000000000000000000")
      await miniChef.fundRewards("1000000000000000000000", 86400);

      await ethers.provider.send("evm_increaseTime", [87000])   // increase evm time by 1 days

      let reward = await yieldWallet.getReward();
          
      await expect(reward).to.emit(pngToken, "Transfer").withArgs(stakingContract, wallet, "999999999999999000000")
      await expect(reward).to.emit(pngToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "199999999999999800000")
      await expect(reward).to.emit(pngToken, "Transfer").withArgs(wallet, signers[0].address, "799999999999999200000")


      await expect(reward).to.emit(yieldWallet, "WithdrawFund").withArgs(pngToken.address, yieldWalletFactory.address, "199999999999999800000");
      await expect(reward).to.emit(yieldWallet, "WithdrawFund").withArgs(pngToken.address, signers[0].address, "799999999999999200000");
    })
    ...
    it('unlock - should emit reward paid event', async function () {
      ...
      await expect(reward).to.emit(miniChef, "Harvest").withArgs(wallet, 0, "999999999999999000000")
    })
  })
  ...
  describe('#claim', async () => {
    ...
    it('should transfer token to user account', async function () {
      ...
      await pngToken.transfer(wallet, "100")
  
      let claim = await yieldwallet.claim(pngToken.address, signers[0].address)
      
      await expect(claim).to.emit(pngToken, "Transfer").withArgs(wallet, yieldWalletFactory.address, "20");
      await expect(claim).to.emit(pngToken, "Transfer").withArgs(wallet, signers[0].address, "80");

      await expect(claim).to.emit(yieldwallet, "WithdrawFund").withArgs(pngToken.address, yieldWalletFactory.address, "20");
      await expect(claim).to.emit(yieldwallet, "WithdrawFund").withArgs(pngToken.address, signers[0].address, "80");
    })
    ...
    it('should emit claim token event', async function () {
      ...
      await expect(
        yieldwallet
          .claim(pngToken.address, signers[0].address)
      ).to.emit(yieldwallet, "Claim")
      .withArgs(pngToken .address, signers[0].address, "80");
    })
  })
