...
describe('PangolinYieldWalletFactory', function () {
  beforeEach(async function () {
    ...
    pngToken = await TestToken.deploy("Pengolin Token", "PNG", 18, signers[0].address)

    let MiniChefV2 = await ethers.getContractFactory('MiniChefV2')
    masterchef = await MiniChefV2.deploy(pngToken.address, signers[0].address)

    await masterchef.addPool("4000", ethDaiPair.address, zeroAddress);

    let PangolinYieldWalletFactory = await ethers.getContractFactory(
      'PangolinYieldWalletFactory'
    )
    yieldWalletFactory = await PangolinYieldWalletFactory.deploy(masterchef.address)
  ...
  describe('#create', async () => {
    ...
    it('should revert if pid is invalid for pool', async function () {
      ...
      await masterchef.addPool("10", pngToken.address, zeroAddress);
