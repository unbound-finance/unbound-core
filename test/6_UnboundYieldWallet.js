...
describe("UnboundYeildWallet", function() {
    ...
    beforeEach(async function () {
        ...
        let UnboundYeildWalletFactory = await ethers.getContractFactory("UnboundYeildWalletFactory");
    ...
    describe("#constructor", async () => {
        ...
        beforeEach(async function(){
            ...
            let UnboundYeildWallet = await ethers.getContractFactory("UnboundYeildWallet");
            yieldwalletInstance = new ethers.Contract(wallet, UnboundYeildWallet.interface.fragments, signers[0]);
        })
    ...
    describe("#deposit", async () => {
        ...
        it("should revert if caller is not vault", async function() { 
            ...
            let UnboundYeildWallet = await ethers.getContractFactory("UnboundYeildWallet");
            let yieldwallet = new ethers.Contract(wallet, UnboundYeildWallet.interface.fragments, signers[0]);

            await expect(yieldwallet.deposit(yieldWalletFactory.address, lockAmount)).to.be.revertedWith("NA")
        });

        it("should increase yeildWalletDeposit amount on lock LPT", async function() { 

            expect(await ethDaiVault.yeildWalletDeposit(signers[0].address)).to.be.equal("0")
        ...
        it("should transfer LPT to yield wallet on lock LPT", async function() { 
            ...
            await ethDaiVault.lock(lockAmount2, signers[0].address, yieldWalletFactory.address, 0)

            let balanceAfter = (new BigNumber(balanceBefore).plus(lockAmount2)).toString()
    ...
    describe("#claim", async () => {
        ...
        it("should revert if caller is not user", async function() { 
            ...
            await expect(yieldwallet.connect(signers[1]).claim(ethDaiPair.address, signers[0].address)).to.be.revertedWith("NA")
        });
