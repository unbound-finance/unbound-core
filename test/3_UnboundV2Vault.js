...
describe("UniswapV2Vault", function() {
    ...
    describe("#constructor", async () => {
        ...
        it("should set the isPeggedToUSD correctly", async function() {
            ...
            expect(await ethDaiVault.isPeggedToUSD(0)).to.equal(isPeggedToUSD0);
            expect(await ethDaiVault.isPeggedToUSD(1)).to.equal(isPeggedToUSD1);
        });
    ...
    describe("#lockWithPermit", async () => {
        ...
        it("should revert if owner doesn't have sufficient lpt balance", async function() { 
            ...
            let ownerLPTBalance = (await ethDaiPair.balanceOf(signers[0].address)).toString();
        ...
        it("should revert if minUTokenAmount is more then minted UND", async function() { 
            ...
            let lptPrice = await calculateLPTPriceFromUniPool(ethDaiPair, tDai.address);

            let totalLPTValueInUSD = (new BigNumber(permitAmount).multipliedBy(lptPrice.toString()).dividedBy(BASE)).toFixed();
            let mintAmount = (new BigNumber(totalLPTValueInUSD).multipliedBy(LTV).dividedBy(secondBase)).toFixed();
        ...
        it("lockWithPermit 1 LP first - should mint correct amount of UND to user account, staking adress & vault contract", async function() { 
            ...
            let protocolFee = (new BigNumber(mintAmount).multipliedBy(PROTOCOL_FEE).dividedBy(secondBase)).toFixed();
            ...
            expect(lock).to.emit(ethDaiPair, "Transfer").withArgs(signers[0].address, ethDaiVault.address, permitAmount);
        ...
        it("lockWithPermit 1 LP - should transfer LPT from user to vault", async function() { 
            let userBalanceBefore = (await ethDaiPair.balanceOf(signers[0].address)).toString()
            let vaultBalanceBefore = (await ethDaiPair.balanceOf(ethDaiVault.address)).toString()
            ...
            let vaultBalanceAfterExpected  = (new BigNumber(vaultBalanceBefore).plus(permitAmount)).toFixed()

            expect(await ethDaiPair.balanceOf(signers[0].address)).to.equal(userBalanceAfterExpected);
            expect(await ethDaiPair.balanceOf(ethDaiVault.address)).to.equal(vaultBalanceAfterExpected);
        })
        ...
        it("should increase collateral and debt amount when locking for second time(different amount) without paying first debt", async function() { 
            ...
            let totalLPTValueInUSD2 = (new BigNumber(permitAmount2).multipliedBy(lptPrice.toString()).dividedBy(BASE)).toFixed();
            let mintAmount2 = (new BigNumber(totalLPTValueInUSD2).multipliedBy(LTV).dividedBy(secondBase)).toFixed();

            let protocolFee2 = (new BigNumber(mintAmount2).multipliedBy(PROTOCOL_FEE).dividedBy(secondBase)).toFixed();
    ...
    describe("#lock", async () => {
        ...
        it("should revert if owner doesn't have sufficient lpt balance", async function() { 
            ...
            await expect(
                ethDaiVault
                .lock(lockAmount, signers[0].address, zeroAddress, "1")
            ).to.be.reverted;
        ...
        it("should revert if mintTo address is zeroAddress", async function() { 
            ...
            await expect(
                ethDaiVault
                .lock(lockAmount, zeroAddress, zeroAddress, "1")
            ).to.be.revertedWith("NO");
        ...
        it("should revert if minUTokenAmount is more then minted UND ", async function() { 
            ...
            let totalLPTValueInUSD = (new BigNumber(lockAmount).multipliedBy(lptPrice.toString()).dividedBy(BASE)).toFixed();
            let mintAmount = (new BigNumber(totalLPTValueInUSD).multipliedBy(LTV).dividedBy(secondBase)).toFixed();
            let minUTokenAmount = new BigNumber(mintAmount).plus("1").toFixed();
        ...
        it("lock 1 LP first - should mint correct amount of UND to user account, staking adress & vault contract", async function() { 
            ...
            let lock = await ethDaiVault.lock(lockAmount, signers[0].address, zeroAddress, finalMintAmount)
                    
            expect(lock).to.emit(und, "Transfer").withArgs(zeroAddress, ethDaiVault.address, protocolFee);
            expect(lock).to.emit(und, "Transfer").withArgs(zeroAddress, undDaiPair, stakeFees);
            expect(lock).to.emit(und, "Transfer").withArgs(zeroAddress, signers[0].address, finalMintAmount);

            expect(lock).to.emit(ethDaiVault, "Lock").withArgs(signers[0].address, lockAmount, finalMintAmount);
        ...
        it("should increase collateral and debt amount when locking for second time(different amount) without paying first debt", async function() { 
            ...
            let totalLPTValueInUSD2 = (new BigNumber(lockAmount2).multipliedBy(lptPrice.toString()).dividedBy(BASE)).toFixed();
            let mintAmount2 = (new BigNumber(totalLPTValueInUSD2).multipliedBy(LTV).dividedBy(secondBase)).toFixed();
...
async function calculateLPTPriceFromUniPool(pair, stablecoin){
    return new Promise(async function(resolve, reject){
        ...
        if(token0.toLowerCase() == stablecoin.toLowerCase()){
            totalPoolValueInDai = new BigNumber(reserve._reserve0.toString()).multipliedBy(2)
        } else {
            totalPoolValueInDai = new BigNumber(reserve._reserve1.toString()).multipliedBy(2)
        }

        resolve(totalPoolValueInDai.multipliedBy(BASE).dividedBy(totalSupply).toFixed())
    })
}